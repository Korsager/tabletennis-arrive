...
import {
  PlayoffMatchRow,
  useReportPlayoffScore,
  usePlayoffMatchGames,
} from "@/hooks/useData";

interface ReportPlayoffScoreModalProps {
  match: PlayoffMatchRow;
  onClose: () => void;
}

const ReportPlayoffScoreModal = ({ match, onClose }: ReportPlayoffScoreModalProps) => {
  ...
  const mutation = useReportPlayoffScore();
  const { data: existingGames } = usePlayoffMatchGames(match.id);
  ...
  const handleSave = () => {
    ...
    if (totals.p1Wins < gamesNeededToWin && totals.p2Wins < gamesNeededToWin) {
      toast.warning("No player has reached the required wins yet — saving anyway.");
    }

    mutation.mutate(
      { playoffMatchId: match.id, games: entered, tournamentId: match.tournament_id },
      {
        onSuccess: () => {
          toast.success("Playoff score reported!");
          onClose();
        },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="mb-2 text-lg font-extrabold text-primary">
          Report Playoff Score — {match.round}
        </h2>
        ...
        <div className="mb-6 space-y-3">
          {games.map((g, idx) => (
            <div key={idx} className="flex items-center gap-3 rounded-lg bg-muted/20 p-3">
              <span className="w-14 text-xs font-semibold text-muted-foreground">Game {idx + 1}:</span>
              <div className="flex flex-1 items-center gap-2">
                <input
                  ...
                  type="number"
                  min={0}
                  className="w-16 rounded-lg border bg-muted/30 px-2 py-2 text-center text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-ring"
                />
        ...
        <div className="mb-6 rounded-lg border border-accent/20 bg-accent/10 p-4">
          <p className="mb-2 text-xs font-semibold text-muted-foreground">Match Score (auto-calculated)</p>
...
export default ReportPlayoffScoreModal;